 ...
 const snapshote= proxy(auth)
  ...
  const sendData=async ()=>{
    try{
        ...
        let respond=await axios.post("http://localhost:5000/login",data)
  ...
  const loading=<div className="spin login-logos"  ><img  alt='spinner' src={spinner}/></div>
    ...
    <div className="container1">
      ...
      <form id="form" action="" method="post" >
        ...
        <fieldset>
          <button name="submit" type="button" id="submit" onClick={submitAction}>submit</button>
          {stats}
          {res? loading : ""}
        </fieldset>
       
        <p className="copyright">Don't have an account? <a href="/signin">signup</a></p>
        <p className="copyright" style={{marginTop: "10px"}}><a href="/" >forgot password?</a></p>
      </form>
